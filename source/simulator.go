...
func main() {
	...
	if Threads > 1 {
		...
		for range Threads - 1 {
			...
			threadChunksSlice = append(threadChunksSlice, &dataChunk{})
		}
		...
		for index := range Threads {
			...
			tempBorderChunk := newBorderChunk(threadChunksSlice[mod(index-1, len(threadChunksSlice))], threadChunksSlice[index])
